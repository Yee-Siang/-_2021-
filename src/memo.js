...
function Memo(props) {
  ...
  const columns = [
    {title: 'State',dataIndex: 'state',key: 'state', width: "8rem", render: (state, record) => (
      state=="Active"?
      <Tag icon={<ClockCircleOutlined/>} onClick={() => {changeState(record.task)}} style={{cursor:"pointer", width: "5.5rem"}}>{state}</Tag>:
      <Tag icon={<CheckCircleOutlined/>} color="success" onClick={() => {changeState(record.task)}} style={{cursor:"pointer", width: "5.5rem"}}>{state}</Tag>)},
    {title: 'Task',dataIndex: 'task',key: 'task', render: (task, record) => (
      record.state=="Active"?
      <span >{task}</span>:
